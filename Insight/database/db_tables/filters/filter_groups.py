...
from ..eve import groups


class Filter_groups(dec_Base.Base, filter_base):
    ...
    channel_id = Column(BIGINT, ForeignKey("discord_channels.channel_id"), primary_key=True, nullable=False,
                        autoincrement=False)
    filter_id = Column(Integer, ForeignKey("groups.group_id"), primary_key=True, nullable=False, autoincrement=False)
    ...
    object_channel = relationship("Channels", uselist=False, back_populates="object_filter_groups")
    object_item = relationship("Groups", uselist=False, lazy="joined")
    ...
    def load_fk_objects(self):
        ...
        self.object_item = groups.Groups(self.filter_id) if self.filter_id else None