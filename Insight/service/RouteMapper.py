...
class SolarSystem(object):
    def __init__(self, rm, system_id, x, y, z):
        ...
        self.y = y
        self.z = z
        self.__gate_distances = {}
        self.__access_counter = 0

    def free_memory(self):
        """if the system has been accessed less than 5 times, free its memory"""
        if 5 >= self.__access_counter:
            self.__gate_distances = {}
            self.__access_counter = 0
    ...
    def load_gate_distances(self):
        try:
            if not self.has_gate_distances():
                self.__gate_distances = nx.single_source_shortest_path_length(self.rm.graph, self)
        except Exception as ex:
            print(ex)

    def has_gate_distances(self):
        """if empty check"""
        return bool(self.__gate_distances)

    def get_distance(self, other):
        if not self.has_gate_distances():
            self.load_gate_distances()
        self.__access_counter += 1
        return self.__gate_distances.get(other)
...
class RouteMapper(object):
    def __init__(self, service_object):
        ...
        self.graph = nx.Graph()
        self.systems = {}
        self.__next_memory_free = datetime.datetime.utcnow()
    ...
    def __memfree(self):
        ...
        try:
            if datetime.datetime.utcnow() >= self.__next_memory_free:
                self.__next_memory_free = datetime.datetime.utcnow() + datetime.timedelta(hours=1)
