...
class TestZk(DatabaseTesting.DatabaseTesting, AsyncTesting.AsyncTesting):
    ...
    def test_01_url_stream(self):
        ...
        if data is None:
            ...
            self.assertIsInstance(v.get("position").get("x"), float)
            self.assertIsInstance(v.get("position").get("y"), float)
            self.assertIsInstance(v.get("position").get("z"), float)
            zkb = data.get("zkb")
            self.assertIsInstance(zkb.get("locationID"), int)
            self.assertIsInstance(zkb.get("hash"), str)
            self.assertIsInstance(zkb.get("fittedValue"), float)
            self.assertIsInstance(zkb.get("totalValue"), float)
            self.assertIsInstance(zkb.get("points"), int)
