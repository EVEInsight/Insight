...
-- -----------------------------------------------------
-- Placeholder table for view `kms_victims`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `kms_victims` (
  `kill_id`            INT,
  `kill_time`          INT,
  `totalValue`         INT,
  `total_involved`     INT,
...
-- -----------------------------------------------------
-- Placeholder table for view `allInvolved_combinedVictim`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `allInvolved_combinedVictim` (
  `involved_pilot_id`    INT,
  `involved_corp_id`     INT,
  `involved_alliance_id` INT
);
...
-- View `kms_victims`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `kms_victims`;
CREATE OR REPLACE VIEW `kms_victims` AS
  SELECT
    iv.kill_id,
...
-- -----------------------------------------------------
-- View `allInvolved_combinedVictim`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `allInvolved_combinedVictim`;
CREATE OR REPLACE VIEW `allInvolved_combinedVictim` AS
  SELECT
    i.character_id   AS involved_pilot_id,
    i.corporation_id AS involved_corp_id,
    i.alliance_id    AS involved_alliance_id,
    v.*
  FROM zk_involved AS i
    INNER JOIN kms_victims AS v ON i.kill_id = v.kill_id;
...
CREATE DEFINER = CURRENT_USER TRIGGER `eve-insight-db`.`discord_EntityFeed_tracking_AFTER_INSERT`
  AFTER INSERT
  ON `discord_EntityFeed_tracking`
  FOR EACH ROW
  BEGIN
    INSERT IGNORE INTO discord_EntityFeed_posted
      SELECT
        tr.EntityFeed_fk,
        kill_id,
        UTC_TIMESTAMP()
      FROM discord_EntityFeed_tracking AS tr
        INNER JOIN allInvolved_combinedVictim AS i
          ON (tr.alliance_tracking IS NOT NULL AND i.involved_alliance_id = tr.alliance_tracking)
      ...
      GROUP BY kill_id;
    INSERT IGNORE INTO discord_EntityFeed_posted VALUES (new.EntityFeed_fk, @k_id, UTC_TIMESTAMP());
  END$$
