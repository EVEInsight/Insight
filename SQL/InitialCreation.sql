...
CREATE TABLE IF NOT EXISTS `discord_CapRadar` (
  ...
  `here_on_capital`       TINYINT NOT NULL DEFAULT 0,
  `here_on_super`         TINYINT NOT NULL DEFAULT 0,
  `everyone_on_capital`   TINYINT NOT NULL DEFAULT 0,
  `everyone_on_super`     TINYINT NOT NULL DEFAULT 0,
  `system_base`           INT     NOT NULL,
...
-- -----------------------------------------------------
-- Table `discord_CapRadar_ignore`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `discord_CapRadar_ignore` (
  `channel_cr`  BIGINT NOT NULL,
  `pilot_id`    INT    NULL DEFAULT NULL,
  `corp_id`     INT    NULL DEFAULT NULL,
  `alliance_id` INT    NULL DEFAULT NULL,
  PRIMARY KEY (`channel_cr`),
  INDEX `pilot_ignore_idx` (`pilot_id` ASC),
  INDEX `corp_ignore_idx` (`corp_id` ASC),
...
CREATE OR REPLACE VIEW `kills_inv_SuperOrCap120M` AS
  SELECT kill_id AS kill_time
  FROM zk_involved
    INNER JOIN zk_kills ON kill_id = killmail_id
    INNER JOIN ships ON ship_type_id = ships.type_id
